...
const sortStarWarsCharacters = (starWarsArr) => {
  // Solution code here...
  const result= starWarsArr.sort( (a,b) => b.height - a.height);
  return result;
};
...
const removeThree = (idx, arr) => {
  // Solution code here...
  arr.splice(idx,3);  //to removes three items in the array starting with the value at the index
  return arr;
};
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(i => {
    const index = i.indexOf(' ',3);
    const toDelete = i.length - index - 1;
    const ingredient = i.slice(-toDelete);
    result.push(ingredient);
  });
  result.reduce( (arr, entry) => {
