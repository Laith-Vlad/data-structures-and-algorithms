...
CHALLENGE 1 - Review
 raisedToTheThird that takes in an array of numbers and returns a new array of each of those numbers raised to the 3rd power (hint: look up Math.pow()). Use forEach to solve this problem.
...
const raisedToTheThird = (arr) => {
  const newArray = [];
  arr.forEach((element) => {
    const result = Math.pow(element, 3);
...
const forLoopTwoToThe = (arr) => {
  ...
  for (const element of arr) {
    const result = Math.pow(2, element);
    newArray.push(result);
  }
  return newArray;
};
