...
function transformToLis(obj){
  ...
  for(let i = 0; i < arr1.length; i++){
    for(let j = 0; j < arr1[i].length-1; j ++){
      arrToReturn.push(`<li>${arr1[i][j]}: ${arr1[i][j+1]}</li>`);
    }
  }
  return arrToReturn;
}
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let finalArToReturn = input.map(item =>item.filter(element => element%5 === 0));
  finalArToReturn = finalArToReturn.map(item => item.filter(element => typeof(element) === 'number'));
  return finalArToReturn.map(item => item.map(element => Math.pow(2, element)));
};
...
let findShortest = (data) => {
  ...
  let shortestName = '';
  data.map(item => {
    if(Number(item.height) < shortest){
      shortest = Number(item.height);
      shortestName = item.name;
    }
  }); return shortestName;
};
